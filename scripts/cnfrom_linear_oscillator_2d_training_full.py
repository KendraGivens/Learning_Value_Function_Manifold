...
def main():
   ...
   # full-horizon training with adaptive sampling
    ...
    # keep total optimization budget the same as staged training
    total_steps = 20000
    ...
    while step < total_steps:
        for batch in train_loader:
            ...
            if step % save_freq == 0 or step == total_steps:
                print(f"\nFull-horizon training step {step}/{total_steps}")
                ...
                for i, eval_tau in enumerate(cfg.tau_schedule):
                    gt_slice = np.array(ground_truth_value_function[i]).T
                    ...
                    plot_linear_oscillator_2d(
                        ...
                        T=cfg.T,
                        scale_time_to_01=cfg.scale_time_to_01,
                        title=f"Predicted vs Ground Truth at tau={eval_tau:.2f}",
                        save_path=plot_dir / f"compare_tau_{eval_tau:.2f}_step_{step:06d}.png",
                    )
